...
def train(model, train_loader, valid_loader, criterion, optimizer, epochs, device):
    ...
    for e in range(epochs):
        ...
        print(f'Training Set - Epoch {e} - Average Loss: {epoch_loss}, Training Epoch Accuracy: {running_corrects}/{running_total} ({100.0 * epoch_acc}%)')
...
def create_data_loaders(train_data_path, valid_data_path, batch_size):
...
def main(args):
    ...
    # Train and validate the model with the data sets
...
if __name__=='__main__':
    ...
    parser.add_argument(
        ...
        default = 64,
        metavar = 'N',
        help = 'Input batch size for training and validation (default: 64)',
    )
    parser.add_argument(
        '--lr', 
        type = float, 
