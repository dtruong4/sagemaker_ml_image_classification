...
def input_fn(request_body, content_type = 'image/jpeg'):
    # Preprocess the JPEG image data
    if content_type == 'image/jpeg': 
        return Image.open(io.BytesIO(request_body))
    
    raise Exception(f'Content type {content_type} is not supported by endpoint.')
...
def model_fn(model_dir):
    ...
    # This model will be used for predictions, so we set it to evaluation mode
    model.eval()
...
def predict_fn(input_object, model):
    ...
    # Preprocess the input object
    input_object = predict_transform(input_object)
    
    # Predict labels for input
    with torch.no_grad():
        prediction = model(input_object.to(device).unsqueeze(0))
...
def output_fn(prediction, accept_type = 'application/json'):
    ...
    raise Exception(f'Content type {accept_type} is not accepted as valid output.')
